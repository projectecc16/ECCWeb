...
public final class TokenHandler {
	...
	public Account parseUserFromToken(String token) {
		...
		if (parts.length == 2 && parts[0].length() > 0 && parts[1].length() > 0) {
			try {
				...
				if (validHash) {
					final Account user = fromJSON(userBytes);
	...
	public String createTokenForUser(Account user) {
	...
	private Account fromJSON(final byte[] userBytes) {
		try {
			return new ObjectMapper().readValue(new ByteArrayInputStream(userBytes), Account.class);
		} catch (IOException e) {
			throw new IllegalStateException(e);
		}
	}

	private byte[] toJSON(Account user) {
		try {
