...
public class CustomMessageConverter extends AbstractHttpMessageConverter<Object>
	 ...
	 */
	public CustomMessageConverter(AccountService accountService) {
	...
	public Object read(Type type, Class<?> contextClass, HttpInputMessage inputMessage)
		...
		JavaType javaType = getJavaType(type, contextClass);
		String myString = IOUtils.toString(inputMessage.getBody(), "UTF-8");
		System.out.println(myString);
		if(myString!=null && myString.contains("crypta")){
			Authentication auth = SecurityContextHolder.getContext().getAuthentication();
			System.out.println(auth.getName());
			Account account = accountService.findByAccountName(auth.getName());
			
			CryptoResource data = objectMapper.readValue(myString, CryptoResource.class);
			System.out.println("SharedKey:"+account.getKeyStore().getSharedKey());
			
			myString = ECCUtil.decryptString(account.getKeyStore().getSharedKey(), data.getCrypta());
			//System.out.println(value);
		}
		//myString = myString.replace("kkk", "vvv");
		
		InputStream in = IOUtils.toInputStream(myString, "UTF-8");
		System.out.println(myString);
		Object obj = readJavaType(javaType, in);
		return obj;
	}

	private Object readJavaType(JavaType javaType, InputStream inputStream) {
		try {
...
//		
//		boolean isEnc = false;
//		if( (outputMessage.getHeaders().get("X-ENC-DATA")!=null) &&
//			(!outputMessage.getHeaders().get("X-ENC-DATA").isEmpty()) && outputMessage.getHeaders().get("X-ENC-DATA").get(0).equalsIgnoreCase("true")){
//			isEnc = true;
//		}
//				
...
//			if(myString!=null && myString.contains("encData")){
//				Authentication auth = SecurityContextHolder.getContext().getAuthentication();
//				System.out.println(auth.getName());
//				Account account = accountService.findByAccountName(auth.getName());
//				
//				CryptoResource data = objectMapper.readValue(myString, CryptoResource.class);
//				System.out.println("SharedKey:"+account.getKeyStore().getSharedKey());
//				System.out.println("DecData:"+data.getDecData());
//				System.out.println("EncData:"+data.getEncData());
//				
//				myString = ECCUtil.decryptString(account.getKeyStore().getSharedKey(), data.getEncData());
//				//System.out.println(value);
//			}
//			String myString = writer.toString();
//			CryptoResource cryptoResource = new CryptoResource
//			myString = ECCUtil.encryptString(account.getKeyStore().getSharedKey(), data.getEncData());
//			
//		}
	...
	protected void writeInternal(Object object, HttpOutputMessage outputMessage)
		...
		if (isEnc){
			try {
				...
				myString = ECCUtil.encryptString(account.getKeyStore().getSharedKey(), myString);
					//System.out.println(value);
				
				CryptoResource res = new CryptoResource(myString);
				object =res;
